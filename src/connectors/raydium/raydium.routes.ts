...
import { executeSwapOptimizedRoute } from './amm-routes/executeSwapOptimized';
import { performanceMetricsRoute } from './amm-routes/performanceMetrics';
...
import { quoteSwapOptimizedRoute } from './amm-routes/quoteSwapOptimized';
import { removeLiquidityRoute as ammRemoveLiquidityRoute } from './amm-routes/removeLiquidity';
import { testRoute } from './amm-routes/testRoute';
...
const raydiumAmmRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  await fastify.register(async (instance) => {
    ...
    await instance.register(executeSwapOptimizedRoute);
    await instance.register(quoteSwapOptimizedRoute);
    await instance.register(ammAddLiquidityRoute);
    await instance.register(ammRemoveLiquidityRoute);
    await instance.register(performanceMetricsRoute);
    await instance.register(testRoute);
  });
};
