  ...
  fastify.get<{
    ...
    async (request) => {
      try {
        ...
        console.log(
          'DEBUG: This tick represents token1/token0 in RAW UNITS (not human readable)',
        );
        ...
        console.log(
          'DEBUG: Expected current tick for price',
          currentHumanPrice,
          ':',
          expectedCurrentTick,
        );
        ...
        console.log(
          'DEBUG: Adjusted lowerTick (after tick spacing):',
          lowerTick,
        );
        console.log(
          'DEBUG: Adjusted upperTick (after tick spacing):',
          upperTick,
        );
        console.log('DEBUG: Pool tick spacing:', tickSpacing);
        console.log('DEBUG: Current pool tick:', pool.tickCurrent);
        console.log(
          'DEBUG: Pool current price (sqrtPriceX96):',
        ...
        console.log(
          'DEBUG: Price from current tick:',
          tickPrice.toSignificant(6),
        );
        console.log(
          'DEBUG: Price from current tick (inverted):',
          tickPrice.invert().toSignificant(6),
        );
        ...
        const isInRange =
          pool.tickCurrent >= lowerTick && pool.tickCurrent <= upperTick;
        ...
        if (baseTokenAmount !== undefined && quoteTokenAmount !== undefined) {
          ...
          const baseRequired = isBaseToken0
            ? position.amount0
            : position.amount1;
          const quoteRequired = isBaseToken0
            ? position.amount1
            : position.amount0;

          const baseRatio =
            parseFloat(baseAmountRaw.toString()) /
            parseFloat(baseRequired.quotient.toString());
          const quoteRatio =
            parseFloat(quoteAmountRaw.toString()) /
            parseFloat(quoteRequired.quotient.toString());
          ...
          if (isBaseToken0) {
            console.log(
              'DEBUG: Creating position from amount0 (base is token0)',
            );
            ...
            console.log(
              'DEBUG: Creating position from amount1 (base is token1)',
            );
          ...
          if (isBaseToken0) {
            console.log(
              'DEBUG: Creating position from amount1 (quote is token1)',
            );
            position = Position.fromAmount1({
              pool,
              tickLower: lowerTick,
            ...
            console.log(
              'DEBUG: Creating position from amount0 (quote is token0)',
            );
            position = Position.fromAmount0({
              pool,
              tickLower: lowerTick,
        ...
        console.log(
          '  - amount0 (raw):',
          actualToken0Amount.quotient.toString(),
        );
        console.log(
          '  - amount1 (raw):',
          actualToken1Amount.quotient.toString(),
        );
        console.log(
          '  - amount0 (formatted):',
          actualToken0Amount.toSignificant(18),
        );
        console.log(
          '  - amount1 (formatted):',
          actualToken1Amount.toSignificant(18),
        );
        console.log(
          '  - mintAmounts.amount0:',
